...
- Enforce the 120-column Swift source-formatting rule from `AGENTS.md`: keep declarations with at most three simple parameters on one line when the complete signature fits; use one-parameter-per-line vertical formatting for longer, four-or-more-parameter, or complex signatures, while keeping each parameter type compact when possible. Keep typed `let`/`var` declarations horizontal when they fit. Permit compact guards only for a simple immediate exit and complete one-line functions only for genuinely trivial pure helpers. Apply the rule only to new or touched declarations unless a separate formatting-only cleanup is approved.
...
- [ ] New and touched Swift declarations follow the 120-column horizontal-first rule; typed bindings stay compact when they fit, and vertical signatures are reserved for long, four-or-more-parameter, or complex declarations.
- [ ] Only guards with a simple immediate exit and genuinely trivial pure helpers use a one-line body; business logic and effectful operations remain multiline.
