...
struct ProductRemoteDataSourceTests {
    ...
    func fetchReturnsConfiguredServerRecords() async throws {
        ...
        #expect(
            try await dataSource.fetchChanges(after: nil)
                == ProductRemoteChangeBatch(records: expectedRecords, nextCursor: ProductSyncCursor(changeSequence: 0))
        )
    }
    ...
    func fetchPropagatesPermissionDenialWithoutProviderTypes() async {
        let dataSource = ProductRemoteDataSourceFake(fetchError: ProductRemoteDataSourceError.permissionDenied)
    ...
    func offlineServerFetchPropagatesUnavailable() async {
        let dataSource = ProductRemoteDataSourceFake(fetchError: ProductRemoteDataSourceError.unavailable)
    ...
    func fetchPreservesInvalidPayloadCodingPath() async {
        ...
        do {
            _ = try JSONDecoder().decode(ProductDTO.self, from: invalidRemoteProductPayload())
...
private actor ProductRemoteDataSourceFake: ProductRemoteDataSource {
    ...
    func fetchChanges(after cursor: ProductSyncCursor?) async throws -> ProductRemoteChangeBatch {
        ...
        return ProductRemoteChangeBatch(records: records, nextCursor: cursor ?? ProductSyncCursor(changeSequence: 0))
    }
...
private func remoteProductDTO() -> ProductDTO {
    ProductDTO(id: "AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE", name: "Ana Alonso", status: .active)
}

private func invalidRemoteProductPayload() -> Data {
    Data(#"{"id":"AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE","name":"Ana Alonso","status":7}"#.utf8)
}
