...
@Suite("Products synchronization engine")
struct ProductSyncEngineTests {
    ...
    func committedPullIsObservableWhenPushFails() async throws {
        ...
        #expect(try await observation.next() == [localProduct])
        let remote = ProductSyncFailingPushRemote(
...
private actor ProductSyncChangeSignalSpy: ProductChangeSignaling {
    private var count = 0
...
private actor ProductSyncDeleteThenFailRemote: ProductRemoteDataSource {
...
private actor ProductSyncPushConflictRemote: ProductRemoteDataSource {
    private let record: ProductRemoteRecord
    private var operationIDs: [UUID] = []
...
private func syncEngineProductID(_ value: String) throws -> ProductID {
    ProductID(rawValue: try syncEngineUUID(value))
}
