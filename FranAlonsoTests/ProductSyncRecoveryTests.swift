...
struct ProductSyncRecoveryTests {
    ...
    func localDeleteHidesProductAndSurvivesRestart() async throws {
        ...
        try dataSource.persistPendingDelete(product.id, operationID: operationID, in: ModelContext(container))
    ...
    func remoteTombstoneWinsOverPendingUpsert() throws {
        ...
        try dataSource.persistPendingUpsert(product, operationID: localOperationID, in: ModelContext(container))
        ...
        try dataSource.reconcileRemoteBatch(
            ProductRemoteChangeBatch(records: [tombstone], nextCursor: ProductSyncCursor(changeSequence: 8)),
            policy: ProductSyncPolicy(),
            in: ModelContext(container)
        )
        ...
        let conflict = try #require(verificationContext.fetch(FetchDescriptor<ProductSyncConflictModel>()).only)
        #expect(try conflict.decodeLocalProduct() == ProductDTO(product))
        #expect(try conflict.decodeRemoteRecord() == tombstone)
        #expect(try dataSource.cursor(in: verificationContext) == ProductSyncCursor(changeSequence: 8))
    }
    ...
    func failedRemoteBatchRollsBackMaterializationAndCursor() throws {
        ...
        let invalidDTO = ProductDTO(id: "not-a-uuid", name: "Invalid identity", status: .active)
        let invalidRecord = ProductRemoteRecord(content: .live(invalidDTO), version: .legacy, changeSequence: nil)
        ...
        #expect(try verificationContext.fetchCount(FetchDescriptor<ProductRemoteStateModel>()) == 0)
    }
    ...
    func staleLiveRecordCannotResurrectTombstone() throws {
        ...
        try dataSource.reconcileRemoteBatch(
            ProductRemoteChangeBatch(records: [tombstone], nextCursor: ProductSyncCursor(changeSequence: 10)),
            policy: ProductSyncPolicy(),
            in: ModelContext(container)
        )
        ...
        try dataSource.reconcileRemoteBatch(
            ProductRemoteChangeBatch(records: [staleLive], nextCursor: ProductSyncCursor(changeSequence: 10)),
            policy: ProductSyncPolicy(),
            in: ModelContext(container)
        )
        ...
        let verificationContext = ModelContext(container)
        #expect(try dataSource.fetchAll(in: verificationContext).isEmpty)
        let state = try #require(verificationContext.fetch(FetchDescriptor<ProductRemoteStateModel>()).only)
    ...
    func pendingOperationIdentityIsGloballyUnique() throws {
        ...
        let secondProduct = Product.testSnapshot(
            id: ProductID(rawValue: recoveryUUID("60000000-0000-0000-0000-000000000099")),
        ...
        #expect(throws: ProductSyncPersistenceError.duplicateOperationIdentity(duplicateOperationID)) {
    ...
    func pendingDeletionBlocksOrdinaryUpsert() throws {
        ...
        #expect(throws: ProductLocalDataSourceError .restoreRequiresExplicitResolution(product.id)) {
            try dataSource.persistPendingUpsert(
                product,
    ...
    func negativePersistedCursorFailsClosed() throws {
        let container = try recoveryContainer()
        let context = ModelContext(container)
        context.insert(ProductSyncCursorModel(feedID: "products", changeSequence: -1))
        try context.save()

        #expect(throws: ProductSyncPersistenceError.invalidCursor) {
            _ = try ProductLocalDataSource().cursor(in: ModelContext(container))
        }
    }
    ...
    func productRecordWithoutFeedMetadataRemainsValidForBootstrap() throws {
        ...
        let record = try JSONDecoder().decode(ProductRemoteRecord.self, from: fixture)
    ...
    func phaseFiveNineClientsStoreReopensWithProductSchema() throws {
        ...
        let clientModel = try #require(context.fetch(FetchDescriptor<ClientModel>()).only)
        #expect(try clientModel.toDomain() == fixture.client)
        let pendingUpsert = try #require(context.fetch(FetchDescriptor<ClientPendingUpsertModel>()).only)
        #expect(try pendingUpsert.decodePayload() == ClientDTO(fixture.client))
        let pendingDelete = try #require(context.fetch(FetchDescriptor<ClientPendingDeleteModel>()).only)
        ...
        let retry = try #require(context.fetch(FetchDescriptor<ClientSyncRetryModel>()).only)
        #expect(try retry.decodeState(for: .pull) == fixture.retryState)
...
private func recoveryProduct(name: String) -> Product {
    Product.testSnapshot(id: ProductID(rawValue: recoveryUUID("60000000-0000-0000-0000-000000000001")), name: name)
}
...
private func writePhaseFiveNineClientStore(at storeURL: URL) throws -> PhaseFiveNineClientFixture {
    ...
    let client = Client.draft(
        id: ClientID(rawValue: recoveryUUID("62000000-0000-0000-0000-000000000001")),
        displayName: "Published 05.9 client"
    )
    let dto = ClientDTO(client)
    let operationID = recoveryUUID("62000000-0000-0000-0000-000000000002")
    let remoteRecord = ClientRemoteRecord(
        content: .live(dto),
        version: .versioned(revision: 9, lastOperationID: recoveryUUID("62000000-0000-0000-0000-000000000003")),
        changeSequence: 9
    )
    ...
    context.insert(
        try ClientPendingDeleteModel(
            ...
            operationID: recoveryUUID("62000000-0000-0000-0000-000000000004"),
    ...
    return PhaseFiveNineClientFixture(client: client, remoteRecord: remoteRecord, retryState: retryState)
}
