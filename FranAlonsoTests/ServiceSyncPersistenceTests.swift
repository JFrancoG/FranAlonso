...
struct ServiceSyncPersistenceTests {
    ...
    func legacyPendingRowKeepsAbsentRemoteBase() throws {
        let serviceID = try syncPersistenceUUID("51000000-0000-0000-0000-000000000001")
        let operationID = try syncPersistenceUUID("51000000-0000-0000-0000-000000000002")
        let service = try makeService(id: serviceID, name: "Pre-sync pending service")
    ...
    func acknowledgingAncestorPreservesDescendantAndVisibleSnapshot() async throws {
        ...
        let serviceID = try syncPersistenceServiceID("52000000-0000-0000-0000-000000000001")
        ...
        let remainingOperations = try verificationContext.fetch(FetchDescriptor<ServicePendingUpsertModel>())
        ...
        let remoteState = try #require(verificationContext.fetch(FetchDescriptor<ServiceRemoteStateModel>()).only)
    ...
    func persistedConflictBlocksOnlyAffectedService() async throws {
        ...
        let conflictedService = try makeService(
            id: try syncPersistenceServiceID("52000000-0000-0000-0000-000000000002").rawValue,
            name: "Conflicted service"
        )
        let unaffectedService = try makeService(
            id: try syncPersistenceServiceID("52000000-0000-0000-0000-000000000003").rawValue,
            name: "Unaffected service"
        )
        let operationID = try syncPersistenceUUID("53000000-0000-0000-0000-000000000003")
        try dataSource.persistPendingUpsert(conflictedService, operationID: operationID, in: ModelContext(container))
        let persistenceActor = ServicePersistenceActor(modelContainer: container)
        let operation = try #require(try await persistenceActor.pendingUpserts().only)
        let concurrentRemote = try makeService(id: conflictedService.id.rawValue, name: "Concurrent remote edit")
        ...
        #expect(throws: ServiceLocalDataSourceError.syncConflictPending(conflictedService.id)) {
            try dataSource.persistPendingUpsert(
                try makeService(id: conflictedService.id.rawValue, name: "Blocked edit"),
        ...
        #expect(deliverableOperations.map(\.serviceID) == [unaffectedService.id.rawValue])
        let verificationContext = ModelContext(container)
        ...
        #expect(try dataSource.fetchAll(in: verificationContext).contains(unaffectedService))
    }
}
