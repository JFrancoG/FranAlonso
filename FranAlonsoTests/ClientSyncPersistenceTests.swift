...
struct ClientSyncPersistenceTests {
    ...
    func legacyPendingRowKeepsAbsentRemoteBase() throws {
        let clientID = try syncPersistenceUUID("51000000-0000-0000-0000-000000000001")
        let operationID = try syncPersistenceUUID("51000000-0000-0000-0000-000000000002")
        let payload = ClientDTO(Client.draft(id: ClientID(rawValue: clientID), displayName: "Pre-sync pending client"))
    ...
    func acknowledgingAncestorPreservesDescendantAndVisibleSnapshot() async throws {
        ...
        let clientID = try syncPersistenceClientID("52000000-0000-0000-0000-000000000001")
        ...
        let remainingOperations = try verificationContext.fetch(FetchDescriptor<ClientPendingUpsertModel>())
        ...
        let remoteState = try #require(verificationContext.fetch(FetchDescriptor<ClientRemoteStateModel>()).only)
    ...
    func persistedConflictBlocksOnlyAffectedClient() async throws {
        ...
        let conflictedClient = Client.draft(
            id: try syncPersistenceClientID("52000000-0000-0000-0000-000000000002"),
            displayName: "Conflicted client"
        )
        let unaffectedClient = Client.draft(
            id: try syncPersistenceClientID("52000000-0000-0000-0000-000000000003"),
            displayName: "Unaffected client"
        )
        let operationID = try syncPersistenceUUID("53000000-0000-0000-0000-000000000003")
        try dataSource.persistPendingUpsert(conflictedClient, operationID: operationID, in: ModelContext(container))
        let persistenceActor = ClientPersistenceActor(modelContainer: container)
        let operation = try #require(try await persistenceActor.pendingUpserts().only)
        let remoteRecord = ClientRemoteRecord(
            client: ClientDTO(Client.draft(id: conflictedClient.id, displayName: "Concurrent remote edit")),
            version: .versioned(
                revision: 4,
        ...
        #expect(throws: ClientLocalDataSourceError.syncConflictPending(conflictedClient.id)) {
            try dataSource.persistPendingUpsert(
                Client.draft(id: conflictedClient.id, displayName: "Blocked edit"),
        ...
        #expect(deliverableOperations.map(\.clientID) == [unaffectedClient.id.rawValue])
        let verificationContext = ModelContext(container)
        ...
        #expect(try dataSource.fetchAll(in: verificationContext).contains(unaffectedClient))
    }
}
