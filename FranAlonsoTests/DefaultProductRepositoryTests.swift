...
struct DefaultProductRepositoryTests {
    ...
    func pendingUpsertPersistsProductAndOperationSnapshotTogether() throws {
        ...
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000001", name: "Ana Alonso")
        let operationID = try repositoryUUID("40000000-0000-0000-0000-000000000001")

        try ProductLocalDataSource().persistPendingUpsert(product, operationID: operationID, in: context)

        let verificationContext = ModelContext(container)
        let operation = try #require(verificationContext.fetch(FetchDescriptor<ProductPendingUpsertModel>()).only)
    ...
    func rejectedSaveRollsBackAttemptedMutation() throws {
        ...
        let storeURL = directoryURL.appending(path: "Products.store", directoryHint: .notDirectory)
        ...
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000009", name: "Rejected write")

        do {
            try ProductLocalDataSource().persistPendingUpsert(
                product,
                operationID: try repositoryUUID("40000000-0000-0000-0000-000000000009"),
                in: context
            )
        ...
        let verificationContext = ModelContext(container)
        #expect(try verificationContext.fetchCount(FetchDescriptor<ProductModel>()) == 0)
    ...
    func existingCallerChangesArePreserved() throws {
        ...
        corruptionContext.insert(
            ProductModel(
                id: try repositoryUUID("30000000-0000-0000-0000-000000000013"),
        ...
        #expect(throws: ProductLocalDataSourceError.contextHasUncommittedChanges) {
            try ProductLocalDataSource().persistPendingUpsert(
                attemptedProduct,
                operationID: try repositoryUUID("40000000-0000-0000-0000-000000000015"),
                in: operationContext
            )
        }

        #expect(operationContext.hasChanges)
        let operationContextIDs = Set(try operationContext.fetch(FetchDescriptor<ProductModel>()).map(\.id))
        ...
        let verificationContext = ModelContext(container)
        let durableProducts = try verificationContext.fetch(FetchDescriptor<ProductModel>())
        #expect(durableProducts.map(\.id) == [try repositoryUUID("30000000-0000-0000-0000-000000000013")])
        #expect(try verificationContext.fetchCount(FetchDescriptor<ProductPendingUpsertModel>()) == 0)
    }
    ...
    func identicalPendingUpsertPreservesOperationIdentity() throws {
        ...
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000002", name: "Same snapshot")
        let originalOperationID = try repositoryUUID("40000000-0000-0000-0000-000000000002")
        let replacementOperationID = try repositoryUUID("40000000-0000-0000-0000-000000000003")
        try dataSource.persistPendingUpsert(product, operationID: originalOperationID, in: ModelContext(container))

        try dataSource.persistPendingUpsert(product, operationID: replacementOperationID, in: ModelContext(container))

        let operation = try #require(ModelContext(container).fetch(FetchDescriptor<ProductPendingUpsertModel>()).only)
    ...
    func changedPendingUpsertAppendsImmutableCausalSuccessor() throws {
        ...
        let initialProduct = try repositoryProduct(id: identifier, name: "Initial snapshot")
        let updatedProduct = try repositoryProduct(id: identifier, name: "Updated snapshot")
        let originalOperationID = try repositoryUUID("40000000-0000-0000-0000-000000000004")
        let updatedOperationID = try repositoryUUID("40000000-0000-0000-0000-000000000005")
        ...
        let verificationContext = ModelContext(container)
        let operations = try verificationContext.fetch(FetchDescriptor<ProductPendingUpsertModel>())
    ...
    func repositoryObservationPublishesItsLocalWrite() async throws {
        ...
        let repository = makeRepository(
            container: container,
            operationID: try repositoryUUID("40000000-0000-0000-0000-000000000006")
        )
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000004", name: "Repository route")
    ...
    func delayedChangeSignalReloadsCurrentSnapshot() async throws {
        ...
        let operationID = try repositoryUUID("40000000-0000-0000-0000-000000000010")
        let repository = DefaultProductRepository(
            persistenceActor: persistenceActor,
            observationSignal: observationSignal,
            operationID: { operationID }
        )
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000010", name: "Newest durable snapshot")
        let stream = await repository.observeProducts()
        var observation = stream.makeAsyncIterator()
        #expect(try await observation.next() == [])

        try await persistenceActor.persistPendingUpsert(product, operationID: operationID)
    ...
    func mappingFailurePreventsCommit() async throws {
        ...
        corruptionContext.insert(
            ProductModel(
                id: try repositoryUUID("30000000-0000-0000-0000-000000000011"),
                name: "Corrupt existing row",
                statusRawValue: "suspended"
            )
        )
        try corruptionContext.save()
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000012", name: "Must not commit")
        let repository = makeRepository(
            container: container,
            operationID: try repositoryUUID("40000000-0000-0000-0000-000000000012")
        )

        await #expect(throws: ProductMappingError.invalidPersistedStatus("suspended")) {
            try await repository.saveProduct(product)
        }

        let verificationContext = ModelContext(container)
        let productModels = try verificationContext.fetch(FetchDescriptor<ProductModel>())
        #expect(productModels.map(\.id) == [try repositoryUUID("30000000-0000-0000-0000-000000000011")])
        #expect(try verificationContext.fetchCount(FetchDescriptor<ProductPendingUpsertModel>()) == 0)
    }
    ...
    func contextualRouteMatchesRepositoryAndUpdatesObservation() async throws {
        ...
        let operationID = try repositoryUUID("40000000-0000-0000-0000-000000000007")
        let repositorySignal = ProductObservationSignal()
        let contextualSignal = ProductObservationSignal()
        let repository = DefaultProductRepository(
            persistenceActor: ProductPersistenceActor(modelContainer: repositoryContainer),
            observationSignal: repositorySignal,
            operationID: { operationID }
        )
        let contextualRepository = DefaultProductRepository(
            persistenceActor: ProductPersistenceActor(modelContainer: contextualContainer),
        ...
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000005", name: "Both routes")
        ...
        try await repository.saveProduct(product)
        try await adapter.save(product, in: contextualContainer.mainContext)
    ...
    func cancellingObservationReleasesPendingIteration() async throws {
        let repository = makeRepository(
            ...
            operationID: try repositoryUUID("40000000-0000-0000-0000-000000000008")
        )
    ...
    func liveDependenciesObserveTheSuppliedSwiftDataContainer() async throws {
        let container = try makeRepositoryContainer()
        let product = try repositoryProduct(id: "30000000-0000-0000-0000-000000000006", name: "Live composition")
        try ProductLocalDataSource().upsert(product, in: ModelContext(container))
...
private func persistedState(in container: ModelContainer) throws -> PersistedProductState {
    let context = ModelContext(container)
    let operation = try #require(context.fetch(FetchDescriptor<ProductPendingUpsertModel>()).only)
...
private func repositoryProduct(id: String, name: String) throws -> Product {
    Product.testSnapshot(id: ProductID(rawValue: try repositoryUUID(id)), name: name)
}
