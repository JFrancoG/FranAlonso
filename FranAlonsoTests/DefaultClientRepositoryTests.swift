...
struct DefaultClientRepositoryTests {
    ...
    func pendingUpsertPersistsClientAndOperationSnapshotTogether() throws {
        ...
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000001", displayName: "Ana Alonso")
        let operationID = try repositoryUUID("40000000-0000-0000-0000-000000000001")

        try ClientLocalDataSource().persistPendingUpsert(client, operationID: operationID, in: context)

        let verificationContext = ModelContext(container)
        let operation = try #require(verificationContext.fetch(FetchDescriptor<ClientPendingUpsertModel>()).only)
    ...
    func rejectedSaveRollsBackAttemptedMutation() throws {
        ...
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000009", displayName: "Rejected write")
    ...
    func existingCallerChangesArePreserved() throws {
        ...
        let operationContextIDs = Set(try operationContext.fetch(FetchDescriptor<ClientModel>()).map(\.id))
        ...
        let verificationContext = ModelContext(container)
        let durableClients = try verificationContext.fetch(FetchDescriptor<ClientModel>())
        #expect(durableClients.map(\.id) == [try repositoryUUID("30000000-0000-0000-0000-000000000013")])
        #expect(try verificationContext.fetchCount(FetchDescriptor<ClientPendingUpsertModel>()) == 0)
    }
    ...
    func identicalPendingUpsertPreservesOperationIdentity() throws {
        ...
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000002", displayName: "Same snapshot")
        ...
        try dataSource.persistPendingUpsert(client, operationID: originalOperationID, in: ModelContext(container))

        try dataSource.persistPendingUpsert(client, operationID: replacementOperationID, in: ModelContext(container))

        let operation = try #require(ModelContext(container).fetch(FetchDescriptor<ClientPendingUpsertModel>()).only)
    ...
    func changedPendingUpsertAppendsImmutableCausalSuccessor() throws {
        ...
        let initialClient = try repositoryClient(id: identifier, displayName: "Initial snapshot")
        let updatedClient = try repositoryClient(id: identifier, displayName: "Updated snapshot")
        ...
        try dataSource.persistPendingUpsert(updatedClient, operationID: updatedOperationID, in: ModelContext(container))

        let verificationContext = ModelContext(container)
        let operations = try verificationContext.fetch(FetchDescriptor<ClientPendingUpsertModel>())
    ...
    func repositoryObservationPublishesItsLocalWrite() async throws {
        ...
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000004", displayName: "Repository route")
    ...
    func delayedChangeSignalReloadsCurrentSnapshot() async throws {
        ...
        try await persistenceActor.persistPendingUpsert(client, operationID: operationID)
    ...
    func mappingFailurePreventsCommit() async throws {
        ...
        try corruptionContext.save()
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000012", displayName: "Must not commit")
        let repository = makeRepository(
            container: container,
            operationID: try repositoryUUID("40000000-0000-0000-0000-000000000012")
        )

        await #expect(throws: ClientMappingError.invalidPersistedStatus("suspended")) {
            try await repository.saveClient(client)
        }

        let verificationContext = ModelContext(container)
        let clientModels = try verificationContext.fetch(FetchDescriptor<ClientModel>())
        #expect(clientModels.map(\.id) == [try repositoryUUID("30000000-0000-0000-0000-000000000011")])
        #expect(try verificationContext.fetchCount(FetchDescriptor<ClientPendingUpsertModel>()) == 0)
    }
    ...
    func contextualRouteMatchesRepositoryAndUpdatesObservation() async throws {
        ...
        let repository = DefaultClientRepository(
            persistenceActor: ClientPersistenceActor(modelContainer: repositoryContainer),
            observationSignal: repositorySignal,
            operationID: { operationID }
        )
        let contextualRepository = DefaultClientRepository(
            persistenceActor: ClientPersistenceActor(modelContainer: contextualContainer),
        ...
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000005", displayName: "Both routes")
        ...
        try await repository.saveClient(client)
        try await adapter.save(client, in: contextualContainer.mainContext)
    ...
    func liveDependenciesObserveTheSuppliedSwiftDataContainer() async throws {
        let container = try makeRepositoryContainer()
        let client = try repositoryClient(id: "30000000-0000-0000-0000-000000000006", displayName: "Live composition")
        try ClientLocalDataSource().upsert(client, in: ModelContext(container))
...
private func persistedState(in container: ModelContainer) throws -> PersistedClientState {
    let context = ModelContext(container)
    let operation = try #require(context.fetch(FetchDescriptor<ClientPendingUpsertModel>()).only)
...
private func repositoryClient(id: String, displayName: String) throws -> Client {
    Client.draft(id: ClientID(rawValue: try repositoryUUID(id)), displayName: displayName)
}
