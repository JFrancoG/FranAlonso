...
struct ProductSyncPersistenceTests {
    ...
    func legacyPendingRowKeepsAbsentRemoteBase() throws {
        let productID = try syncPersistenceUUID("51000000-0000-0000-0000-000000000001")
        let operationID = try syncPersistenceUUID("51000000-0000-0000-0000-000000000002")
        let payload = ProductDTO(
            Product.testSnapshot(id: ProductID(rawValue: productID), name: "Pre-sync pending product")
        )
    ...
    func acknowledgingAncestorPreservesDescendantAndVisibleSnapshot() async throws {
        ...
        let productID = try syncPersistenceProductID("52000000-0000-0000-0000-000000000001")
        ...
        let ancestorOperationID = try syncPersistenceUUID("53000000-0000-0000-0000-000000000001")
        let descendantOperationID = try syncPersistenceUUID("53000000-0000-0000-0000-000000000002")
        try dataSource.persistPendingUpsert(ancestor, operationID: ancestorOperationID, in: ModelContext(container))
        ...
        try dataSource.persistPendingUpsert(descendant, operationID: descendantOperationID, in: container.mainContext)
        let acknowledgedRecord = ProductRemoteRecord(
            product: ProductDTO(ancestor),
            version: .versioned(revision: 1, lastOperationID: ancestorOperationID)
        )

        try await persistenceActor.acknowledge(operationID: ancestorOperationID, record: acknowledgedRecord)

        let verificationContext = ModelContext(container)
        let remainingOperations = try verificationContext.fetch(FetchDescriptor<ProductPendingUpsertModel>())
        ...
        #expect(try dataSource.fetchAll(in: verificationContext) == [descendant])
        let remoteState = try #require(verificationContext.fetch(FetchDescriptor<ProductRemoteStateModel>()).only)
    ...
    func persistedConflictBlocksOnlyAffectedProduct() async throws {
        ...
        let conflictedProduct = Product.testSnapshot(
            id: try syncPersistenceProductID("52000000-0000-0000-0000-000000000002"),
            name: "Conflicted product"
        )
        let unaffectedProduct = Product.testSnapshot(
            id: try syncPersistenceProductID("52000000-0000-0000-0000-000000000003"),
            name: "Unaffected product"
        )
        let operationID = try syncPersistenceUUID("53000000-0000-0000-0000-000000000003")
        try dataSource.persistPendingUpsert(conflictedProduct, operationID: operationID, in: ModelContext(container))
        let persistenceActor = ProductPersistenceActor(modelContainer: container)
        let operation = try #require(try await persistenceActor.pendingUpserts().only)
        let remoteRecord = ProductRemoteRecord(
            product: ProductDTO(Product.testSnapshot(id: conflictedProduct.id, name: "Concurrent remote edit")),
            version: .versioned(
                revision: 4,
                lastOperationID: try syncPersistenceUUID("53000000-0000-0000-0000-000000000004")
            )
        )
        ...
        #expect(throws: ProductLocalDataSourceError.syncConflictPending(conflictedProduct.id)) {
            try dataSource.persistPendingUpsert(
                Product.testSnapshot(id: conflictedProduct.id, name: "Blocked edit"),
                operationID: try syncPersistenceUUID("53000000-0000-0000-0000-000000000005"),
        ...
        try dataSource.persistPendingUpsert(
            unaffectedProduct,
            operationID: try syncPersistenceUUID("53000000-0000-0000-0000-000000000006"),
            in: ModelContext(container)
        )
        let deliverableOperations = try await persistenceActor
            .deliverablePendingUpserts()
        #expect(deliverableOperations.map(\.productID) == [unaffectedProduct.id.rawValue])
        let verificationContext = ModelContext(container)
        ...
        #expect(try dataSource.fetchAll(in: verificationContext).contains(unaffectedProduct))
    }
}
