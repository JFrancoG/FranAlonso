...
extension ProductDTO {
    ...
    init(_ product: Product) {
        ...
        self.init(id: product.id.rawValue.uuidString, name: product.name, status: status)
    }
    ...
    func toDomain() throws -> Product {
        guard let identifier = UUID(uuidString: id) else { throw ProductMappingError.invalidIdentifier(id) }
        let domainStatus: ProductStatus = switch status {
        case .active: .active
        case .inactive: .inactive
        }

        return Product(id: ProductID(rawValue: identifier), name: name, status: domainStatus)
    }
}
