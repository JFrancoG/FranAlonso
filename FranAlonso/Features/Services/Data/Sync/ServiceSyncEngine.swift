...
/// Reconciles the durable Services operation chain with the provider-neutral remote source.
actor ServiceSyncEngine {
    private let persistenceActor: ServicePersistenceActor
    private let remoteDataSource: any ServiceRemoteDataSource
    private let observationSignal: any ServiceChangeSignaling
    private let syncPolicy: ServiceSyncPolicy
    private let retryPolicy: SyncBackoffPolicy
    private let timing: SyncTiming
    private var isSynchronizing = false

    /// Creates the Services engine from isolated local and replaceable remote roles.
    init(
        persistenceActor: ServicePersistenceActor,
        remoteDataSource: any ServiceRemoteDataSource,
        observationSignal: any ServiceChangeSignaling,
        policy: ServiceSyncPolicy = ServiceSyncPolicy(),
    ...
    /// automatic trigger in this Service checkpoint.
    ///
    /// - Throws: `ServiceSyncEngineError.alreadySynchronizing` for an overlapping pass,
    ///   `CancellationError` when the caller cancels, or a remote, policy or persistence error.
    func synchronize() async throws {
        guard !isSynchronizing else { throw ServiceSyncEngineError.alreadySynchronizing }
        ...
        var conflictedServiceIDs: Set<UUID> = []
        for operation in operations {
            if conflictedServiceIDs.contains(operation.serviceID),
    ...
    private func reconcilePushed(
        ...
        result: ServiceRemoteMutationResult,
        ...
        case .conflict(let reason, let remoteRecord):
            guard case .upsert(let upsert) = operation else { throw ServiceSyncPersistenceError.entityIdentityMismatch }
    ...
    private func performWithRetry<Success: Sendable>(
        ...
        for attempt in 1...3 {
            ...
            do {
                ...
                let remoteError = error as? ServiceRemoteDataSourceError
...
/// Failures owned by the Services synchronization pass rather than its dependencies.
enum ServiceSyncEngineError: Error, Equatable {
