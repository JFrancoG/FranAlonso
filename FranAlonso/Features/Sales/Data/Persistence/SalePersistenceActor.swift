...
/// Serializes Sales persistence on a SwiftData-owned model context.
///
...
actor SalePersistenceActor {
    ...
    func fetchAll() throws -> [Sale] {
        try dataSource.fetchAll(in: modelContext)
    }

    /// Inserts or replaces a sale by stable identity and saves the actor's context.
    ///
    /// - Parameter sale: The detached Domain value to persist.
    /// - Throws: A SwiftData fetch or save error.
    func upsert(_ sale: Sale) throws {
        try dataSource.upsert(sale, in: modelContext)
    }

    /// Commits a sale and one pending remote upsert on this actor's context.
    ///
    /// - Parameters:
    ...
    func deliverablePendingUpserts() throws -> [SalePendingUpsert] {
        try dataSource.deliverablePendingUpserts(in: modelContext)
    }

    /// Returns operations eligible for delivery under the whole-snapshot conflict policy.
    func deliverablePendingOperations() throws -> [SalePendingOperation] {
    ...
    func cursor() throws -> SaleSyncCursor? {
    ...
    func recordRemoteObservation(_ record: SaleRemoteRecord) throws {
        try dataSource.recordRemoteObservation(record, in: modelContext)
    }

    /// Persists both sides of a Sales conflict for later explicit resolution.
    func recordConflict(
