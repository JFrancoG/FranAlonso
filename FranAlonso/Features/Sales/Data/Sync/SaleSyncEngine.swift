...
/// Reconciles the durable Sales operation chain with the provider-neutral remote source.
actor SaleSyncEngine {
    private let persistenceActor: SalePersistenceActor
    private let remoteDataSource: any SaleRemoteDataSource
    private let observationSignal: any SaleChangeSignaling
    private let syncPolicy: SaleSyncPolicy
    private let retryPolicy: SyncBackoffPolicy
    private let timing: SyncTiming
    private var isSynchronizing = false

    /// Creates the Sales engine from isolated local and replaceable remote roles.
    init(
        persistenceActor: SalePersistenceActor,
        remoteDataSource: any SaleRemoteDataSource,
        observationSignal: any SaleChangeSignaling,
        policy: SaleSyncPolicy = SaleSyncPolicy(),
    ...
    /// automatic trigger in this Sale checkpoint.
    ///
    ...
    func synchronize() async throws {
        ...
        var conflictedSaleIDs: Set<UUID> = []
        for operation in operations {
            if conflictedSaleIDs.contains(operation.saleID),
            ...
            if case .conflict = result {
                conflictedSaleIDs.insert(operation.saleID)
            }
            await observationSignal.publishChange()
        }
    }
    ...
    private func reconcilePushed(
        ...
        result: SaleRemoteMutationResult,
    ...
    private func performWithRetry<Success: Sendable>(
        ...
        for attempt in 1...3 {
            ...
            do {
                ...
                let remoteError = error as? SaleRemoteDataSourceError
...
/// Failures owned by the Sales synchronization pass rather than its dependencies.
enum SaleSyncEngineError: Error, Equatable {
