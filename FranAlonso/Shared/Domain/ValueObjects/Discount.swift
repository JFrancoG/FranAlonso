...
extension Discount {
    ...
    init(percentage: Decimal) throws {
        guard !percentage.isNaN, (Decimal.zero ... 100).contains(percentage) else { throw DiscountError.outOfRange }
    ...
    init(from decoder: any Decoder) throws {
        ...
        try self.init(percentage: container.decode(Decimal.self, forKey: .percentage))
    }
